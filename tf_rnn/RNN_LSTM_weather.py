...
def genData_W(load_weekday, temperature, humidity, pressure, n_train, n_valid, n_lag, T, curr_day):
    ...
    X_test[0, :, 1] = temperature[curr_day*T - n_lag*T: curr_day*T]
...
def RNN(x, weights, biases, num_hidden, timesteps, num_input):
...
def RNN_LSTM(load, curr_day):    
    ...
    (X_train, y_train, X_valid, y_valid, X_test, y_test, min_load, max_load) = genData_W(load, temperature, humidity, pressure, n_train, n_valid, n_lag, T, curr_day)
